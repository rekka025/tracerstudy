        ...
        if ($this->upload->do_upload('importexcel')) {
            ...
            foreach ($reader->getSheetIterator() as $sheet) {
                foreach ($sheet->getRowIterator() as $row) {
                        $data_alumni = array(
                            'nim'  => $row->getCellAtIndex(0),
                            'nik'     => $row->getcellatindex(1),
                            'kd_prodi'     => $row->getcellatindex(2),
                            'nama'     => $row->getcellatindex(3),
                            'jenis_kelamin'  => $row->getcellatindex(4),
                            'tempat_lahir'  => $row->getcellatindex(5),
                            'tanggal_lahir'  => $row->getcellatindex(6),
                            'hp'  => $row->getcellatindex(7),
                            'tahun_lulus'  => $row->getcellatindex(8),
                            'npwp'  => $row->getcellatindex(9),  
                        );
